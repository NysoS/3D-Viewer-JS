...
class MeshLoader {
  ...
  static async loadFromArray(content: any): Promise<Mesh> {
    let mesh = new Mesh();
    let vertices: Vector3D[] = [];

    console.log(content);

    content.forEach((raw: string) => {
      this.parse(raw, mesh, vertices);
    });
  ...
  static parse(raw: string, mesh: Mesh, vertices: Vector3D[]): void {
    if (raw.startsWith("v ")) {
